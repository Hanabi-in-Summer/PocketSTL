...
namespace pocket_stl{
    ...
    public:
        ...
        static_assert(std::is_same<charT, typename traits_type::char_type>::value,
                "charT must be same as traits_type::char_type");
    ...
    public:
        ...
        basic_string& replace(const_iterator first, const_iterator last, const basic_string& str){
            assert(begin() <= first && last <= end() && first <= last);
    ...
    insert(const_iterator pos, Iter first, Iter last)
    {
        ...
        uninitialized_copy(first, last, r);
        size_ += len;
        return r;
    }
    ...
    template <class charT, class traits, class Alloc>
    int basic_string<charT, traits, Alloc>::
    ...
    compare(size_type pos1, size_type count1, const basic_string& other) const{
        auto n1 = std::min(count1, size_ - pos1);
    ...
    compare(size_type pos1, size_type count1, const basic_string& other,
        ...
        auto n1 = std::min(count1, size_ - pos1);
        auto n2 = std::min(count2, other.size_ - pos2);
    ...
    // to_raw_pointer 函数
    template <class charT, class traits, class Alloc>
    typename basic_string<charT, traits, Alloc>::const_pointer
    basic_string<charT, traits, Alloc>::
    ...
    append_range(Iter first, Iter last){
        ...
        pocket_stl::uninitialized_copy_n(first, n, buffer_ + size_);
        size_ += n;
        return *this;
    }
    ...
    reallocate(size_type need){
        const auto new_cap = std::max(cap_() + need, cap_() + (cap_() >> 1));
    ...
    reallocate_and_fill(iterator pos, size_type n, value_type ch){
        ...
        const auto new_cap = std::max(old_cap + n, old_cap + (old_cap >> 1));
        auto new_buffer = data_allocator().allocate(new_cap);
    ...
    reallocate_and_copy(iterator pos, const_iterator first, const_iterator last){
        ...
        auto new_buffer = data_allocator().allocate(new_cap);
        auto e1 = char_traits::move(new_buffer, buffer_, r) + r;
        auto e2 = pocket_stl::uninitialized_copy_n(first, n, e1) + n;
        char_traits::move(e2, buffer_ + r, size_ - r);
        data_allocator().deallocate(buffer_, old_cap);
        buffer_ = new_buffer;
        size_ += n;
    ...
    template <class charT, class traits, class Alloc>
    basic_string<charT, traits, Alloc>
    operator+(charT ch, const basic_string<charT, traits, Alloc>& rhs){
        basic_string<charT, traits, Alloc> tmp(1, ch);
        tmp.append(rhs);
        return tmp;
    }
    ...
    template <class charT, class traits, class Alloc>
    basic_string<charT, traits, Alloc>
    operator+(const basic_string<charT, traits, Alloc>& lhs, charT ch){
        basic_string<charT, traits, Alloc> tmp(lhs);
        tmp.append(1, ch);
        return tmp;
    }
    ...
    operator+(basic_string<charT, traits, Alloc>&& lhs,
            const basic_string<charT, traits, Alloc>& rhs){
        basic_string<charT, traits, Alloc> tmp(std::move(lhs));
        tmp.append(rhs);
        return tmp;
    }
    ...
    operator+(const basic_string<charT, traits, Alloc>& lhs,
        ...
        basic_string<charT, traits, Alloc> tmp(std::move(rhs));
    ...
    operator+(const charT* lhs, basic_string<charT, traits, Alloc>&& rhs){
        basic_string<charT, traits, Alloc> tmp(std::move(rhs));
        tmp.insert(tmp.begin(), lhs, lhs + char_traits<charT>::length(lhs));
        return tmp;
    }

    template <class charT, class traits, class Alloc>
    basic_string<charT, traits, Alloc>
    operator+(charT ch, basic_string<charT, traits, Alloc>&& rhs){
        basic_string<charT, traits, Alloc> tmp(std::move(rhs));
    ...
    template <class charT, class traits, class Alloc>
    basic_string<charT, traits, Alloc>
    operator+(basic_string<charT, traits, Alloc>&& lhs, charT ch){
        basic_string<charT, traits, Alloc> tmp(std::move(lhs));
        tmp.append(1, ch);
        return tmp;
    }
    ...
    void swap(basic_string<charT, traits, Alloc>& lhs,
            basic_string<charT, traits, Alloc>& rhs) noexcept{
        lhs.swap(rhs);
    }

    // 特化 mystl::hash
    template <class charT, class traits, class Alloc>
    struct pocket_stl::hash<basic_string<charT, traits, Alloc>>{
        size_t operator()(const basic_string<charT, traits, Alloc>& str)
        {
